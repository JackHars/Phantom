...
static bool map_status(UIState *s){
  return s->fb_w == 930 ? true : false;
}

//Stop prob
static void ui_stop_prob(UIState *s){
  float stop_line = (*s->sm)["modelV2"].getModelV2().getStopLine().getProb() * 100;
  int stop = (int) stop_line;
  const std::string probstr = std::to_string((int)std::nearbyint(stop));
  const int center_x = s->fb_w/2;
  const int center_y = s->fb_h/2;
  const int bottom_y = center_y * 1.67;
  nvgBeginPath(s->vg);
  nvgRoundedRect(s->vg, center_x / 2, bottom_y, center_x, 75, 35);
  nvgFillColor(s->vg, nvgRGBA(0,0,0,100));
  nvgFill(s->vg);
  nvgClosePath(s->vg);
  ui_draw_text(s, center_x - 30, bottom_y + 50, "Stop Probability: %", 50, nvgRGBA(255,255,255,255), "sans-bold");
  ui_draw_text(s, center_x + 170 , bottom_y + 50, probstr.c_str(), 50, nvgRGBA(255,255,255,255), "sans-bold");
  if (!map_status(s)){
    nvgBeginPath(s->vg);
    NVGpaint imgPaint = nvgImagePattern(s->vg, center_x / 2, bottom_y, 80, 80, 0, s->images.at("stop"), stop_line > 90 ? 1 : 0.1);
    nvgRect(s->vg, center_x / 2, bottom_y, center_x, 75);
...
static void ui_draw_vision_long(UIState *s) {
  if (!map_status(s)){
    const Rect rect = {bdr_s * 9, int(bdr_s * 1.5), 184, 202};
    ui_fill_rect(s->vg, rect, COLOR_BLACK_ALPHA(100), 30.);
    ui_draw_rect(s->vg, rect, COLOR_WHITE_ALPHA(100), 10, 20.);
    nvgTextAlign(s->vg, NVG_ALIGN_CENTER | NVG_ALIGN_BASELINE);
    ui_draw_text(s, rect.centerX(), 118, "LONG", 26 * 2.5, COLOR_WHITE_ALPHA(200), "sans-regular");
    if (s->scene.longitudinal_control) {
    const std::string longstr = "OP";
    ui_draw_text(s, rect.centerX(), 212, longstr.c_str(), 48 * 2.5, COLOR_WHITE, "sans-bold");
  } else {
    ui_draw_text(s, rect.centerX(), 212, "STOCK", 25 * 2.5, COLOR_WHITE_ALPHA(100), "sans-semibold");
  }
  }
  else {
    const Rect rect = {bdr_s * 2, int(bdr_s * 9), 184, 202};
    ui_fill_rect(s->vg, rect, COLOR_BLACK_ALPHA(100), 30.);
    ui_draw_rect(s->vg, rect, COLOR_WHITE_ALPHA(100), 10, 20.);
    nvgTextAlign(s->vg, NVG_ALIGN_CENTER | NVG_ALIGN_BASELINE);
    ui_draw_text(s, rect.centerX(), 340, "LONG", 26 * 2.5, COLOR_WHITE_ALPHA(200), "sans-regular");
    if (s->scene.longitudinal_control) {
    const std::string longstr = "OP";
    ui_draw_text(s, rect.centerX(), 434, longstr.c_str(), 48 * 2.5, COLOR_WHITE, "sans-bold");
  } else {
    ui_draw_text(s, rect.centerX(), 434, "STOCK", 25 * 2.5, COLOR_WHITE_ALPHA(100), "sans-semibold");
  }
  }
}
...
static void ui_draw_vision_speed(UIState *s) {
  ...
  ui_draw_text(s, s->fb_w/2, 1000, "On branch: Experimental", 15 * 2.5, nvgRGBA(2,138,242,255), "sans-regular");
}
...
static void ui_draw_vision_header(UIState *s) {
  ...
  ui_draw_vision_long(s);
  if (!map_status(s)){ui_draw_modelbox(s);}
  ui_stop_prob(s);
...
static void ui_draw_vision(UIState *s) {
  ...
  if ((*s->sm)["controlsState"].getControlsState().getAlertSize() == cereal::ControlsState::AlertSize::NONE && !map_status(s)) {
