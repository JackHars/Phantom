...
PrimeAdWidget::PrimeAdWidget(QWidget* parent) : QFrame(parent) {
  ...
  QLabel *upgrade = new QLabel("Phantom-ci");
  upgrade->setStyleSheet("font-size: 75px; font-weight: bold;");
  main_layout->addWidget(upgrade, 0, Qt::AlignTop);
  main_layout->addSpacing(50);

  QLabel *description = new QLabel("Autonomous technology for Toyota Corolla");
  ...
  QLabel *features = new QLabel("TO DO:");
  features->setStyleSheet("font-size: 41px; font-weight: bold; color: #E5E5E5;");
  main_layout->addWidget(features, 0, Qt::AlignBottom);
  main_layout->addSpacing(30);

  QVector<QString> bullets = {"Remote commands", "Finish nav integration", "Add MQB support"};
  for (auto &b: bullets) {
